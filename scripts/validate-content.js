...
for (const [key, country] of european) {
  if (!country.alpha2 || !country.capital || !country.currencyCode || !country.lang || !country.bestTime) {
    errors.push(`Europa ${key}: dados básicos incompletos`);
  }
  const entry = country.entryRequirements;
  if (!entry || !['verified', 'needs-review'].includes(entry.status) || !official(entry.officialSource) || !date(entry.verifiedAt)) {
    errors.push(`Entrada ${key}: fonte, data ou status inválido`);
  }
  if (!entry?.ees?.status || !entry?.etias?.status) errors.push(`Europa ${key}: EES/ETIAS ausente`);
}
...
console.log(`Cartões: ${CARD_OFFERS.length}; redes VIP: ${LOUNGE_TYPES.length}; aeroportos: ${AIRPORT_GUIDES.length}; salas: ${AIRPORT_GUIDES.reduce((n, airport) => n + airport.lounges.length, 0)}; upgrades: ${AIRLINE_UPGRADE_EXAMPLES.length}; países europeus: ${european.length}; marcos: ${landmarks.length}; marcos needs-review: ${needsReview}; cartões com campos a confirmar: ${uncertainCards}; erros: ${errors.length}`);
